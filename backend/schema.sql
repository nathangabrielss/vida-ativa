-- Schema do Sistema Vida Ativa (participantes de saúde/bem-estar).
-- Espelhado em migrations/0001_schema.sql (D1) e functions/_lib/schema.js.
...
-- Perfil + dados de saúde do participante (seção 5). Idade é derivada de
-- data_nascimento, nunca armazenada. user_id liga a um login opcional.
...
-- Avaliações físicas (seção 6). Histórico imutável: só INSERT, nunca UPDATE.
-- imc e imc_classificacao são calculados no servidor ao salvar.
...
-- Biblioteca de vídeos/materiais educativos (seção 11).
